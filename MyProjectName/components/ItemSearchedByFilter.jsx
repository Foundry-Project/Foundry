...
import { BASE_URL } from '../wifiip.js'; // Uncomment if you need the base URL
import { format } from 'date-fns';
import { useNavigation } from '@react-navigation/native';

const ItemSearchedByFilter = () => {
  const { filreddata, setfilreddata, userid } = useAppContext();
  const [checkedItems, setCheckedItems] = useState({});
  const navigation = useNavigation(); // Initialize the navigation

  useEffect(() => {
    const matchedItems = filreddata.reduce((acc, match) => {
