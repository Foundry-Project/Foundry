...
const NavBar = ({ navigation }) => {
  const { selectedIcon, setSelectedIcon, notifications } = useAppContext(); // Use context

  const handleIconPress = (iconName, navigateTo) => {
    setSelectedIcon(iconName); // Update selected icon in context
    navigation.navigate(navigateTo); // Navigate to the specified screen
  };

  const hasNotifications = notifications.length > 0; // Check if there are notifications

  return (
    <SafeAreaView style={styles.menuContainer}>
      <View style={styles.iconContainer}>
        <TouchableOpacity onPress={() => handleIconPress('Home', 'HomePage')}>
          <HomeIcon color={selectedIcon === 'Home' ? '#C9FDD7' : 'white'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleIconPress('Found', 'FoundPage')}>
          <FoundIcon color={selectedIcon === 'Found' ? '#C9FDD7' : 'white'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleIconPress('Add', 'AddPage')}>
          <AddIcons color={selectedIcon === 'Add' ? '#C9FDD7' : 'white'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleIconPress('Notifications', 'NotificationsPage')}>
          <NotifIcons color={selectedIcon === 'Notifications' ? '#C9FDD7' : 'white'} hasNotifications={hasNotifications} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleIconPress('Profile', 'Setti')}>
          <ProfileIcon color={selectedIcon === 'Profile' ? '#C9FDD7' : 'white'} />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
