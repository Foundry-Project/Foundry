...
// import Payy from 'react-native-vector-icons/MaterialIcons';
...
const Settings = ({ navigation }) => {
  ...
  const handleNavigation = (icon) => {
    if (icon === user) {
      ...
      navigation.navigate('PaymentPage');  // Navigate to PaymentPage
    }
  };
        ...
        <View style={{ marginHorizontal: '5%', marginVertical: '10%' }}>
          ...
          <View style={styles.optionsContainer}>
            {[
              ...
              { icon: Termsand, label: 'Terms & Conditions' },
              { icon: Priva, label: 'Privacy Policy' },
              // Added Payment option here
            ].map((item, index) => (
              <Pressable
                key={index}
                style={styles.optionItem}
                onPress={() => handleNavigation(item.icon)}
              >
                <Image source={item.icon} style={styles.optionIcon} />
                <Text style={styles.optionText}>{item.label}</Text>
                <Arrow name="arrow-forward-ios" size={30} color={"grey"} />
...
const styles = StyleSheet.create({
  scrollContainer: {
    flexGrow: 1,
    paddingBottom: 100, // Adjust to prevent NavBar overlap
  },
  profileImage: {
    width: '20%',
    height: undefined,
